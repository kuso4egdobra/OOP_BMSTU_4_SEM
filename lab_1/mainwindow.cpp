...
{   
    ...
    ERRORS err = task_manager(IMPORT_DEFAULT_POINTS, connector, window);
    if (err != SUCCES_TO_IMPORT_POINTS)
        show_message(err, this);
    else
    {
        task_manager(UPDATE_SCENE, connector, window);
        QMessageBox::information(this, "", "Ось ОZ направлена от наблюдателя "
                                 "и \nпроходит через пересечение осей OX и OY."
                                 "\n\nРассматриваемая геометричесая фигура -  "
                                 "усеченная пирамида.");
    }
}
...
void MainWindow::on_Button_set_default_points_clicked()
{
    ERRORS err = task_manager(IMPORT_DEFAULT_POINTS, connector, window);
